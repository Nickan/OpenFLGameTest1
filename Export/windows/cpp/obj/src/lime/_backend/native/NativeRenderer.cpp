...
Void NativeRenderer_obj::create( ){
{
		...
		if ((this->useHardware)){
			...
			this->render();
			HX_STACK_LINE(55)
			::lime::graphics::RenderContext _g3 = ::lime::graphics::RenderContext_obj::CAIRO(this->cairo);		HX_STACK_VAR(_g3,"_g3");
			HX_STACK_LINE(55)
			this->parent->context = _g3;
		}
	}
return null();
}
...
{
		HX_STACK_FRAME("lime._backend.native.NativeRenderer","render",0x996f6573,"lime._backend.native.NativeRenderer.render","lime/_backend/native/NativeRenderer.hx",93,0x398cc96c)
		HX_STACK_THIS(this)
		HX_STACK_LINE(93)
		if ((!(this->useHardware))){
			HX_STACK_LINE(96)
			...
			if (((bool((bool((bool((this->cacheLock == null())) || bool((this->cacheLock->__Field(HX_CSTRING("pixels"),true) != lock->__Field(HX_CSTRING("pixels"),true))))) || bool((this->cacheLock->__Field(HX_CSTRING("width"),true) != lock->__Field(HX_CSTRING("width"),true))))) || bool((this->cacheLock->__Field(HX_CSTRING("height"),true) != lock->__Field(HX_CSTRING("height"),true)))))){
				...
				if (((this->cairo != null()))){
					HX_STACK_LINE(107)
					this->cairo->recreate(this->primarySurface);
				}
				else{
					HX_STACK_LINE(111)
